...
async function loginUser(username, password) {
  username = (username || '').trim().toLowerCase();
  // Intentar con el dominio original primero, luego con el alternativo
  const domains = ['@gestor.hogar', '@gestor.hogar.app'];
  for (const domain of domains) {
    try {
      ...
      // Si es el último dominio, devolver el error
      if (domain === domains[domains.length - 1]) {
        return { ok: false, error: _fbError(e.code) };
      }
      // Si no es "user not found", no intentar otro dominio
...
function openSettings() {
  ...
  // Telegram
  const tgToken  = document.getElementById('settings-telegram-token');
  const tgChatId = document.getElementById('settings-telegram-chat-id');
  if (tgToken)  tgToken.value  = state.telegramToken  || localStorage.getItem('hogar_telegram_token')  || '';
  if (tgChatId) tgChatId.value = state.telegramChatId || localStorage.getItem('hogar_telegram_chatid') || '';
...
function saveSettings() {
  ...
  const tgToken  = document.getElementById('settings-telegram-token');
  const tgChatId = document.getElementById('settings-telegram-chat-id');
  const tokenVal  = tgToken  ? tgToken.value.trim()  : '';
  const chatIdVal = tgChatId ? tgChatId.value.trim() : '';
  state.telegramToken  = tokenVal;
  state.telegramChatId = chatIdVal;
  localStorage.setItem('hogar_telegram_token',  tokenVal);
  localStorage.setItem('hogar_telegram_chatid', chatIdVal);
  saveState();
  _updateSettingsStatus();
...
function clearApiKey() {
  ...
  localStorage.removeItem('hogar_telegram_token');
  localStorage.removeItem('hogar_telegram_chatid');
  saveState();
  const keyInput = document.getElementById('settings-api-key');
  ...
  if (tgChatId) tgChatId.value = '';
  _updateSettingsStatus();
  showToast('Claves eliminadas');
}
...
function _updateSettingsStatus() {
  ...
  // Telegram status
  const tgStatusEl = document.getElementById('settings-telegram-status');
  if (!tgStatusEl) return;
  const tgToken = state.telegramToken || localStorage.getItem('hogar_telegram_token') || '';
...
function openGroupsModal() {
  ...
  function emojiBtn(onclickExpr, current) {
    return EMOJI_PALETTE.map(e =>
      `<button type="button" onclick="${onclickExpr.replace('__E__', e)}" style="border:${e===current?'2px solid var(--primary)':'1px solid var(--border)'};background:var(--bg);border-radius:6px;padding:3px 5px;font-size:1.1rem;cursor:pointer;">${e}</button>`
    ).join('');
  }
  ...
  function renderGroupsBody() {
    ...
    return `
      ...
      <div style="display:flex;gap:0.4rem;align-items:center;">
        <button type="button" id="new-group-emoji-btn" onclick="toggleEmojiPicker('new')"
          style="width:3rem;height:2.2rem;text-align:center;font-size:1.2rem;background:var(--bg);border:1px solid var(--border);border-radius:6px;cursor:pointer;">${esc(newEmoji)}</button>
      ...
      ${openNew ? `<div style="display:flex;flex-wrap:wrap;gap:4px;margin-top:6px;padding:6px;background:var(--bg-soft,rgba(0,0,0,0.15));border-radius:6px;">${emojiBtn("pickNewGroupEmoji('__E__')", newEmoji)}</div>` : ''}
...
window.pickNewGroupEmoji = function(e) {
  window._newGroupEmoji = e;
...
window.addCustomGroup = function() {
  if (!state.customGroups) state.customGroups = [];
  const emoji = window._newGroupEmoji || '🏷️';
...
let _telegramLastUpdateId = parseInt(localStorage.getItem('hogar_tg_last_update') || '0', 10);

function startTelegramPolling() {
  stopTelegramPolling();
  const token = state.telegramToken || localStorage.getItem('hogar_telegram_token') || '';
  if (!token) return;
  _telegramPollTimer = setInterval(() => pollTelegramUpdates(token), 10000);
  pollTelegramUpdates(token);
}

function stopTelegramPolling() {
  if (_telegramPollTimer) { clearInterval(_telegramPollTimer); _telegramPollTimer = null; }
}
...
async function pollTelegramUpdates(token) {
  ...
  try {
    ...
    for (const update of data.result) {
      _telegramLastUpdateId = update.update_id;
      localStorage.setItem('hogar_tg_last_update', String(_telegramLastUpdateId));
      ...
      if (gastoMatch) {
        ...
        const existing = state.months[currentMonth].expense.find(r => r.name.toLowerCase() === desc.toLowerCase());
        if (existing) {
          existing.value = String((parseFloat(existing.value) || 0) + monto);
          if (who) existing.who = who;
        } else {
          state.months[currentMonth].expense.push({
            name: desc, value: String(monto), who: who, commerce: '', paymentMethod: '', group: ''
          });
        }
        saveState();
        renderMain();

        // Confirmar al usuario via Telegram
        try {
          await fetch('https://api.telegram.org/bot' + token + '/sendMessage', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        showToast('Gasto desde Telegram: $' + monto + ' — ' + desc);
        continue;
      }
      ...
      if (text === '/saldo' || text === '/balance') {
        if (state.months[currentMonth]) {
          const totalIncome  = sum(state.months[currentMonth].income);
          const totalExpense = sum(state.months[currentMonth].expense);
          const balance = totalIncome - totalExpense;
          try {
            await fetch('https://api.telegram.org/bot' + token + '/sendMessage', {
              method: 'POST',
      ...
      // Comando /help
      if (text === '/help' || text === '/start' || text === '/ayuda') {
        try {
          await fetch('https://api.telegram.org/bot' + token + '/sendMessage', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
